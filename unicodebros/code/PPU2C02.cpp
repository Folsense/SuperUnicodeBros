...
PPU2C02::PPU2C02(){
    ...
    paletteRead.open("ntscpalette.pal");
    std::cout << "Reading\n";
    unsigned char val;
    float val2;
    char buffer[192];
    ...
    if(scanline == -1){
        ...
        if(cycle < 340){
            ...
            if(cycle == 2){
                ...
                doNMI2 = true;
                draw();
                long thisTime = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() - startTime;
                int toSleep;
                int toSleep2;
                if(numTimes > 59){
                ...
                //}
                lastTime = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() - startTime;
